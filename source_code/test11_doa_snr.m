...
%% 加载原始数据（和test4的数据相同）
load test11_data/doa/G2.mat 
load test11_data/doa/Sigma2_x.mat
load test11_data/doa/Sigma2_y.mat 
load test11_data/doa/Sigma2_z.mat
% G2 = G2.^2;
% Sigma2_x=Sigma2_x.^2;
% Sigma2_y = Sigma2_y.^2;
% Sigma2_z= Sigma2_z.^2;
...
% end
% figure
% [c,handle]=contour(x_span,y_span,sqrt(Sigma2_z(:,:,1)),20);
...
% NAN_ratio_list = {'0.99','0.93'};
for index_list = 1:1:length(ERR_list)%TTTTTTTTTTTTTTTTTT
CHOSEN_IDX_LIST = [];
snr=str2num(ERR_list{index_list});
NAN_ratio = 0.8;%TTTTTTTTTTTTTTTTTT
...
% Algorithm_list = {'GeneralGDOP','GeneralGDOP2'};
...
save_path = ['test11_data/doa/',ContrastAlgorithm,'/fs_10_snr_',ERR_list{index_list},'/'];
...
if strcmp(ContrastAlgorithm,'GeneralGDOP')||strcmp(ContrastAlgorithm,'GeneralGDOP2')
    ...
    if ~strcmp(ContrastAlgorithm,'GeneralGDOP')
        snr_i = 20-(NAN_ratio-0.8)*50;
        sigma_theta1 = sigma_theta1+sigma_theta1/sqrt(snr_i)*randn;%10dB信噪比
        sigma_theta2 = sigma_theta2+sigma_theta2/sqrt(snr_i)*randn;%10dB信噪比
        sigma_s_esti = sigma_s_esti+sigma_s_esti/sqrt(snr_i)*randn;%10dB信噪比
    end
    for k =1:1:length(z_span)
        for i = 1:1:length(x_span)
            for j = 1:1:length(y_span)
    ...
    noise = sigma_e.*randn(size(G2));
    recon_tensor = recon_tensor+noise;%0.01km标准差的噪声
    % snr_para = -150*NAN_ratio+170;
    % sigma_theta1 = awgn(0.1,snr_para);
    % sigma_theta2 = awgn(0.08,snr_para);
    % sigma_s = awgn(1e-3,snr_para);
    % for k =1:1:length(z_span)
    %     for i = 1:1:length(x_span)
    %         for j = 1:1:length(y_span)
    %             m=x_span(i)+0.01;
    ...
    for i = 1:1:size(recon_tensor,3)
        frob_list = [frob_list,frob(recon_tensor(:,:,i)-sqrt(G2(:,:,i)))];
    end
...
elseif strcmp(ContrastAlgorithm,'Kriging')
    ...
    for i =1:1:length(z_indices_unique)
        ...
        S = meas_matrix_i(:,1:2);
        Y = meas_matrix_i(:,4);
        theta = [2 2]; lob = [1e-1 1e-1]; upb = [50 50];
        theta = [2 2]; lob = [10 10]; upb = [50 50];
        %变异函数模型为高斯模型
        [dmodel, ~] = dacefit(S, Y, @regpoly1, @corrgauss, theta, lob, upb);
 ...
 elseif strcmp(ContrastAlgorithm,'SVT')
    ...
    elseif strcmp(ContrastAlgorithm,'RBF')
        ...
        for i =1:1:length(z_indices_unique)
            ...
            Rs = 150;
            z3=RBF2(X0,Y0,values,X,Y,'gaussian',Rs,2,'rbf',0.1);
