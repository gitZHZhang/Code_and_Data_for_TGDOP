...
for alg_i = 1:1:length(Algorithm_list)
    ...
    if strcmp(Algorithm,'GeneralGDOP')
        ...
        % % clabel(c,handle);
        % recon_tensor = load ('test18_data\GeneralGDOP\G_cal.mat').G_cal;
...
save([dir_pos,'rate=',num2str(train_rate),'/','time_list.mat'],'time_list')
save([dir_pos,'rate=',num2str(train_rate),'/','ERR_list.mat'],'ERR_list')
save([dir_pos,'rate=',num2str(train_rate),'/','recon_rate_list.mat'],'recon_rate_list')
end
disp('finish')
