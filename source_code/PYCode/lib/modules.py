...
class RadioWNet(nn.Module):
    ...
    def __init__(self,inputs=2,phase="firstU"):
        ...
        self.layer3 = convrelu(100, 150, 5, 1,2) 
        ...
        self.conv_up3 = convreluT(150 + 150, 100, 4, 0) 
        ...
        self.Wlayer3 = convrelu(70, 90, 5, 1,2) 
        ...
        self.Wconv_up3 = convreluT(90 + 90, 70, 4, 0) 
...
class MatrixReconstructionViT(nn.Module):
    def __init__(self, 
                 img_size=128,      # 矩阵的边长 (假设是正方形)
                 patch_size=16,     # 切片大小
                 in_channels=1,     # 矩阵通道数 (通常二维矩阵为1)
                 embed_dim=256,     # Transformer 内部特征维度
        ...
        # -----------------------------------------------------------
        # A. Patch Embedding 层
        # 使用 Conv2d 实现: stride=patch_size 可以直接提取 patch
        # -----------------------------------------------------------
        self.patch_embed = nn.Conv2d(in_channels, embed_dim, 
                                     kernel_size=patch_size, stride=patch_size)

        # -----------------------------------------------------------
        # B. 位置编码 (Positional Embedding)
        # -----------------------------------------------------------
        ...
        # -----------------------------------------------------------
        # D. Reconstruction Head (解码头)
        # 将 latent vector 映射回 patch 的原始像素值
        # -----------------------------------------------------------
        self.head = nn.Linear(embed_dim, self.patch_dim)

    def forward(self, x):
        """
        x: (Batch, Channel, Height, Width) - 包含0的incomplete矩阵
        """
        B, C, H, W = x.shape
        ...
        # 4. Predict Pixels (Reconstruction)
        # (B, Num_Patches, Embed_Dim) -> (B, Num_Patches, Patch_Size*Patch_Size*C)
        x = self.head(x)

        # 5. Unpatchify (将 Patch 序列还原回 2D 图像)
        x_rec = self.unpatchify(x)
        
        return x_rec
    def unpatchify(self, x):
        """
        将 (B, N, P*P*C) 还原为 (B, C, H, W)
        """
        p = self.patch_size
        h = w = int(self.img_size // p)
