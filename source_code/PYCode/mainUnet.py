...
if __name__ == '__main__':
    ...
    abs_path = 'E:\\zzh清华\\GDOP张量分析\\张量分解和稀疏重构\\0-电磁频谱重构\\论文\\code2\\'+scene+'\\tdoa1\\UNet\\'
    # abs_path = 'E:\\zzh清华\\GDOP张量分析\\张量分解和稀疏重构\\0-电磁频谱重构\\论文\\code2\\test15_data\\tdoa1\\UNet\\'
    ...
    # -----------------------------------------------------------------------------------------------------
        ...
        if mode=='train':
            ...
            #3-第二次UNet训练#################################
            model =modules.RadioWNet(inputs=1,phase="secondU")
            model.load_state_dict(torch.load(read_path+'model/Trained_Model_FirstU.pt'))
            model.to(device)
            optimizer_ft = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-4)
            ...
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            model =modules.RadioWNet(inputs=1,phase="secondU")
            model.load_state_dict(torch.load(model_path+'model/Trained_Model_secondU.pt'))
            print("The model {} has been loaded!".format(model_path+'model/Trained_Model_secondU.pt'))
            model.to(device)
            model.eval()   # Set model to evaluate mode
            metrics = defaultdict(float)
