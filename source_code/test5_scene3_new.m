...
syms M
%% 最小二乘解析解情况：将sigmati估计值作为已知量，将其余变量作为未知量求解
%% 0-给出各变量的真值
...
% for snr_idx = 1:1:length(SNR_LIST)-1
    ...
    save_path = ['test5_data/scene3/err_',num2str(err),'_SNR_',num2str(SNR_PARA),'/'];
    ...
    for iter = 1:1:Mont_times
        ...
        sigmat1_initial = sigmat1_real+sigmat1_real*10^(-SNR_PARA/20)*randn;
        ...
        for i=1:1:size(meas_matrix,1)
            ...
            G_Cal = subs(G_eq, sigmat1,sigmat1_initial);
    ...
    res_all = [sigmat2_list;sigmat3_list;sigmas_list;eta12_list;eta13_list;eta23_list];
