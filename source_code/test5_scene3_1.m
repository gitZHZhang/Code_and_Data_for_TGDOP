...
n_G_num_list = [20,100,200,400,600,1000];
n_samples_list = [20,100,200,400,600,1000];
for iter_G = 1:1:length(n_G_num_list)
    [sigmat1_list,sigmat2_list,sigmat3_list,sigmas_list,eta12_list,eta13_list,eta23_list] = deal([]);%用于存放不同iter的估计值
    for iter = 1:1:iter_N
        SNR = 20;
        sigmat1_initial = mean(sigmat1_real+sigmat1_real*10^(-SNR/20)*randn(10,1));
        ...
        for i=1:length(x)/fs_x
            ...
            for j=1:length(y)/fs_y
                for k=1:1:length(z)/fs_z
                    ...
                    % n_G_num是变化的
                    n_samples = 10;%假设G是由1000个样本统计平均得到的
                    n_G_num= n_G_num_list(iter_G); %假设G的测量值有1000次统计结果
%                     % n_samples是变化的
%                     n_samples = n_samples_list(iter_G);
%                     n_G_num = 10;
                    ...
                    %                 n_samples = 20;%假设G是由1000个样本统计平均得到的
                    %                 n_G_num= ceil(2*G_real^2/n_samples*10); %假设G的测量值有1000次统计结果
                    %                 var_G = 2*G_real^2/n_samples/n_G_num; %测量到的G的方差
                    %                 G_real = mean(G_real + sqrt(var_G)*randn(n_G_num,1)); %带噪G
                    ...
                    G_Cal = subs(G_Cal, sigmat1,sigmat1_real); %在表达式中代入变量和预设值，得到计算的G
%                     G_Cal_list = [G_Cal_list;G_Cal];
    ...
    savepath = ['scene3/1噪声影响分析/G有噪/方案1/n_G_num=',num2str(n_G_num)];
