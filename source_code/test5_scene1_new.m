...
% for snr_idx = 1:1:length(SNR_LIST)-1
    ...
    save_path = ['test5_data/scene1/err_',num2str(err),'_SNR_',num2str(SNR_PARA),'/'];
    ...
    for iter = 1:1:Mont_times
        ...
        disp(disp_str)
        sigmas_initial = real_value(4)+real_value(4)*10^(-SNR_PARA/20)*randn;
        % var_value_cal1 = [sigmat1,sigmat2,sigmat3,sigmas_real,eta12,eta13,eta23];%实际代入公式计算的变量列表
        var_value_cal1 = [sigmat1,sigmat2,sigmat3,sigmas_initial,eta12,eta13,eta23];%实际代入公式计算的变量列表
        coeffs = [];
        constant = [];
        b = [];
    ...
    res_all = [sigmat1_list;sigmat2_list;sigmat3_list;eta12_list;eta13_list;eta23_list];
