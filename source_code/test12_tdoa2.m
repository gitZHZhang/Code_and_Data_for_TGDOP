...
%% 加载原始数据（和test4的数据相同）
load test12_data/tdoa2/G2.mat 
load test12_data/tdoa2/Sigma2_x.mat
load test12_data/tdoa2/Sigma2_y.mat 
load test12_data/tdoa2/Sigma2_z.mat
%% 传感器位置
L=30;
xt = 0;yt = 0;zt = 0;
x1 = L*cos(10*pi/180);y1 = L*sin(10*pi/180);z1=0.1;
x2 = L*cos(170*pi/180);y2 = L*sin(170*pi/180);z2 = 0.2;
x3 = 0;y3 = -L;z3 = 0.3;
...
%     end
% end
vars_real = [30e-3,20e-3,10e-3,5e-3,0.3,0.2,0.1];
CHOSEN_IDX_LIST = [];
Mont_times = 15;
...
save_path = ['test12_data/tdoa2/',ContrastAlgorithm,'/fs_10_ratio_',num2str(NAN_ratio),'/'];
    ...
    for k =1:1:length(z_span)
        for i = 1:1:length(x_span)
            for j = 1:1:length(y_span)
                ...
                [Gx,Gy,Gz,G] = cal_gdop_doa(emitter,sensor,value);
                recon_tensor(j,i,k) = G;
                Sigma2_x_hat(j,i,k) = Gx;
                Sigma2_y_hat(j,i,k) = Gy;
                Sigma2_z_hat(j,i,k) = Gz;
            end
        end
    end
...
elseif strcmp(ContrastAlgorithm,'Kriging')
    ...
    frob_list.errG = frob_listx+frob_listy+frob_listz;
    frob_list.errGx = frob_listx;
    frob_list.errGy = frob_listy;
    frob_list.errGz = frob_listz;
    recon_T.G = Sigma2_x_hat+Sigma2_y_hat+Sigma2_z_hat;
    recon_T.Gx = Sigma2_x_hat;
    recon_T.Gy = Sigma2_y_hat;
    recon_T.Gz = Sigma2_z_hat;
elseif strcmp(ContrastAlgorithm,'RBF')
    ...
    for p=1:1:3
        for i =1:1:length(z_indices_unique)
            ...
            [X,Y]=meshgrid(-399:800/size(G2,1):400);
            %二维RBF插值
            if NAN_ratio>=95
                Rs = 50;
            else
