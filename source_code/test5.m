...
% % 判断不同高度的G有什么区别：先下降再上升
% y=-400:10:400;x=-400:10:400;z=linspace(80,90,10);
...
%     end
%     if k>1
%         res(k) = sum(sum(G(:,:,k)-G(:,:,k-1)));
%     end
% end
%% 计算GDOP的三维空间分布&验证公式正确性
cal_z = 1;%是否计算三维的gdop
y=-400:1:400;x=-400:1:400;z=linspace(10,20,11);
% y=-100:0.25:100;x=-100:0.25:100;z=1:2:21;
% z=10;
[gdop,G,sigma2_x,sigma2_y,sigma2_z,gdop2_TDOA,gdop2_S] = deal(zeros(length(x),length(y),length(z)));
for i=1:length(x)
    for j=1:length(y)
...
%             end
            if(cal_z)
                [gdop_ji,gdop2_TDOA_ji,gdop2_S_ji,sigma2_x_ji,sigma2_y_ji,sigma2_z_ji,g_ji] = cal_gdop2(emitter,sensor);
                % vars = [30e-3,30e-3,30e-3,5e-3,0.3,0.3,0.3];
                % [Gx_eq,Gy_eq,Gz_eq,G_eq] =
                % cal_diff(emitter,sensor,vars);%时间单位us，距离单位km，和上式效果相同
            else
...
real_value = [30e-3, 30e-3, 30e-3,5e-3, 0.2, 0.2, 0.2];%变量对应的真实值标签，可以看到eta并不是0
...
sigmas_initial = awgn(real_value(4),20,'measured');
...
var_value_cal1 = [sigma_t1_initial,sigma_t2_initial,sigma_t3_initial,sigmas_initial,eta12,eta12,eta12];%实际代入公式计算的变量列表
coeffs = [];
b = [];
for i=1:length(x)/fs_x
    disp(['processing sigmat(s)_',num2str(i)])
    for j=1:length(y)/fs_y
        for k=1:1:length(z)/fs_z            
            ...
            coeff(1) = double(subs(G_Cal, var_name,0));%求常数项式系数
            coeff(2) = double(subs(G_Cal, var_name,1))-coeff(1);%求1次项式系数
            coeffs =  [coeffs;coeff];
