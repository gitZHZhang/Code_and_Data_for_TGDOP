close all
clear all
% scene2：任一cov(sigmati,sigmatj)有相对准确的估计值时，可以优化出其余eta、sigmat和sigmas等变量的参数
...
for iter = 1:1:iter_N
    SNR = 20;
    cov12_real = eta12_real*sigmat1_real*sigmat2_real;
    cov12_initial = mean(cov12_real+cov12_real*10^(-SNR/20)*randn(10,1));
    ...
    for i=1:length(x)/fs_x
        ...
        for j=1:length(y)/fs_y
            for k=1:1:length(z)/fs_z
                ...
                %             G_real = awgn(G_real,20,'measured');%加噪
                G_Cal = subs(G_eq, var_value,var_value_cal1); %在表达式中代入变量和预设值，得到计算的G
                G_Cal = subs(G_Cal, eta12*sigmat1*sigmat2,cov12_initial); %在表达式中代入变量和预设值，得到计算的G
                
                var_name = [sigmat1,sigmat2,sigmat3,sigmas,eta13,eta23];%G的表达式中有哪些多项式的项
                ...
                %             coeffs =  [coeffs;coeff];
                var_name = [sigmat1^2,sigmat2^2,sigmat3^2,sigmas^2,eta13*sigmat1*sigmat3,eta23*sigmat2*sigmat3];%G的表达式中有哪些多项式的项
    ...
    sigmas_res = sqrt(res1(4));sigmas_list = [sigmas_list,sigmas_res];
    eta12_res = cov12_initial/sigmt1_res/sigmt2_res; eta12_list = [eta12_list,eta12_res];
...
files = dir('scene2/*.mat');
for i=1:1:length(files)
    str = ['scene2/',files(i).name];
    load(str)
end
...
% legend('sigmat1-real','sigmat1-esti','sigmat2-real','sigmat2-esti','sigmat3-real','sigmat3-esti','sigmas-real','sigmas-esti')
leg = legend('$\sigma_{\Delta t_1}$','$\tilde{\sigma}_{\Delta t_1}$','$\sigma_{\Delta t_2}$','$\tilde{\sigma}_{\Delta t_2}$',...
    '$\sigma_{\Delta t_3}$','$\tilde{\sigma}_{\Delta t_3}$','$\sigma_s$','$\tilde{\sigma}_s$');
