function [gdop,gdop2_TDOA,gdop2_S,P_err] = cal_gdop(emitter,sensor)
m = emitter(1);n = emitter(2);
x_row = num2cell(sensor(:,1));y_col = num2cell(sensor(:,2));
[xt,x1,x2,x3] = deal(x_row{:});
[yt,y1,y2,y3] = deal(y_col{:});
r1=((m-x1).^2+(n-y1).^2).^(1/2);
r2=((m-x2).^2+(n-y2).^2).^(1/2);
r3=((m-x3).^2+(n-y3).^2).^(1/2);
rt=((m-xt).^2+(n-yt).^2).^(1/2);
c11=(m-x1)/r1;c21=(m-x2)/r2;c31=(m-x3)/r3;ct1=(m-xt)/rt;
c12=(n-y1)/r1;c22=(n-y2)/r2;c32=(n-y3)/r3;ct2=(n-yt)/rt;
c=[(-ct1+c11) (-ct2+c12) ;(-ct1+c21) (-ct2+c22) ;(-ct1+c31) (-ct2+c32)];
b=pinv(c);
sigmap=3e5*30e-9;
% sigmap = 1;
sigmas = 5e-3;
