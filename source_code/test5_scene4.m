...
for iter = 1:1:iter_N
    SNR = 20;
    eta12_initial = mean(eta12_real+eta12_real*10^(-SNR/20)*randn(10,1));
    eta13_initial = mean(eta13_real+eta13_real*10^(-SNR/20)*randn(10,1));
    eta23_initial = mean(eta23_real+eta23_real*10^(-SNR/20)*randn(10,1));
    ...
    for i=1:length(x)/fs_x
        ...
        for j=1:length(y)/fs_y
            for k=1:1:length(z)/fs_z
                ...
                G_Cal = subs(G_Cal, [eta12,eta13,eta23],[eta12_initial,eta13_initial,eta23_initial]); %在表达式中代入变量和预设值，得到计算的G
