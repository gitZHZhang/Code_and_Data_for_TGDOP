...
target_span{1}=[-400,400,-400,400,10,20];
target_span{2}=[-300,300,-300,300,10,20];
target_span{3}=[-200,200,-200,200,10,20];
target_span{4}=[-100,100,-100,100,10,20];
target_span{5}=[-50,50,-50,50,10,20];
...
for iter_G = 1:1:length(target_span)
    y=linspace(target_span{iter_G}(3),target_span{iter_G}(4),801);
    x=linspace(target_span{iter_G}(1),target_span{iter_G}(2),801);
    z=linspace(target_span{iter_G}(5),target_span{iter_G}(6),11);%感兴趣范围
    rel_err = 1;
        ...
        for i=1:length(x)/fs_x
            ...
            for j=1:length(y)/fs_y
                for k=1:1:length(z)/fs_z
                    ...
                    n_samples = 30;%假设G是由1000个样本统计平均得到的
                    n_G_num= 30; %假设G的测量值有1000次统计结果
    ...
    savepath = ['scene3/3标校源位置分析/span=',num2str(target_span{iter_G}(2))];
