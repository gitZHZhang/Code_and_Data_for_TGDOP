...
%% 加载原始数据（和test4的数据相同）
load test16_data/tdoa1/G2.mat 
load test16_data/tdoa1/Sigma2_x.mat
load test16_data/tdoa1/Sigma2_y.mat 
load test16_data/tdoa1/Sigma2_z.mat
%% 传感器位置
xt = 0;yt = 0;zt = 0;
...
sub_Sigma2_x = Sigma2_x(41:81, 41:81, 1:11);
sub_Sigma2_y = Sigma2_y(41:81, 41:81, 1:11);
sub_Sigma2_z = Sigma2_z(41:81, 41:81, 1:11);

vars_real = [18e-3,20e-3,25e-3,0.5,-0.3,0.5,-0.2]; %us us us m
CHOSEN_IDX_LIST = [];
CHOSEN_IDX_LIST2 = [];
Mont_times = 10;
NAN_ratio_list = {'0.8','0.85','0.9','0.95','0.99'};
% NAN_ratio_list = {'0.99'};
...
save_path = ['test16_data/tdoa1/',ContrastAlgorithm,'/fs_10_ratio_',num2str(NAN_ratio),'/'];
...
incomplete_Tx = Sigma2_x+ERR*randn(size(Sigma2_x));%0.01km标准差的噪声
incomplete_Ty = Sigma2_y+ERR*randn(size(Sigma2_y));%0.01km标准差的噪声
incomplete_Tz = Sigma2_z+ERR*randn(size(Sigma2_z));%0.01km标准差的噪声
...
incomplete_Tx1 = incomplete_Tx; incomplete_Ty1 = incomplete_Ty; incomplete_Tz1 = incomplete_Tz;
incomplete_Tx1(41:81,41:81,1:11)=NaN; incomplete_Ty1(41:81,41:81,1:11)=NaN; incomplete_Tz1(41:81,41:81,1:11)=NaN;

incomplete_Tx2 = sub_Sigma2_x+ERR*randn(size(sub_Sigma2_x));
incomplete_Ty2 = sub_Sigma2_y+ERR*randn(size(sub_Sigma2_y));
incomplete_Tz2 = sub_Sigma2_z+ERR*randn(size(sub_Sigma2_z));
incomplete_Tx2(CHOSEN_IDX2) = NaN;
incomplete_Ty2(CHOSEN_IDX2) = NaN;
incomplete_Tz2(CHOSEN_IDX2) = NaN;

if strcmp(ContrastAlgorithm,'Proposed')
    incomplete_Tx1 = fmt(incomplete_Tx1);
    incomplete_Ty1 = fmt(incomplete_Ty1);
    incomplete_Tz1 = fmt(incomplete_Tz1);
    L1 = [8 8 3];
    L2 = [8 8 3];
    L3 = [8 8 3];
    poly=12; %
    type = 'polyBTD_nonneg'; %typelist:typicalBTD_nonneg/ typicalBTD/ polyBTD_nonneg/ polyBTD
    [sol,output] = runBTD_MultiInput({incomplete_Tx1,incomplete_Ty1,incomplete_Tz1},[L1;L2;L3],poly,type);
    ...
    incomplete_Tx2 = fmt(incomplete_Tx2);
    incomplete_Ty2 = fmt(incomplete_Ty2);
    incomplete_Tz2 = fmt(incomplete_Tz2);
    [sol,output] = runBTD_MultiInput({incomplete_Tx2,incomplete_Ty2,incomplete_Tz2},[L1;L2;L3],poly,type);
    [A1_res,B1_res,C1_res,S1_res,A2_res,B2_res,C2_res,S2_res,A3_res,B3_res,C3_res,S3_res] = deal(sol.factors{:});
    ...
    for i = 1:1:size(recon_tensor,3)
        frob_list1 = [frob_list1,frob(sqrt(recon_tensor(:,:,i))-sqrt(G2(:,:,i)))];
        frob_listx = [frob_listx,frob(sqrt(Sigma2_x_hat(:,:,i))-sqrt(Sigma2_x(:,:,i)))];
        frob_listy = [frob_listy,frob(sqrt(Sigma2_y_hat(:,:,i))-sqrt(Sigma2_y(:,:,i)))];
        frob_listz = [frob_listz,frob(sqrt(Sigma2_z_hat(:,:,i))-sqrt(Sigma2_z(:,:,i)))];
    end
