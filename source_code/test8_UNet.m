...
disp(['Generating the train data with NAN_ratio=',num2str(NAN_ratio)])
...
%% 2-生成神经网络测试集
if strcmp(mode,'test')
disp(['Generating the test data with NAN_ratio=',num2str(NAN_ratio)])
CHOSEN_IDX_LIST = [];
for i=1:1:Mont_times
    CHOSEN_IDX_LIST = [CHOSEN_IDX_LIST;randperm(numel(G2),round(NAN_ratio*numel(G2)))];
end
...
disp(['Evaluating the test data with saved model and NAN_ratio=',num2str(NAN_ratio)])
frob_list_all_Mont = [];
ContrastAlgorithm=Algorithm_list{1};
for i=1:1:size(G2,3)
    read_path = ['test8_data/',ContrastAlgorithm,'/fs_10_ratio_',num2str(NAN_ratio),'/','outdata/z_',num2str(i)];
...
save_path = ['test8_data/UNet/fs_10_ratio_',num2str(NAN_ratio),'/'];
