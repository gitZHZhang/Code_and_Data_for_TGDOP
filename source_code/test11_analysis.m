...
for scene_idx = 1:1:length(scene_list)
    ...
    load_path0 = ['test11_data/',scene_i,'/'];
    if analysis==1
        ...
        for i = 1:1:length(frob_list_algorithms)
            ...
            % box_i = boxplot(group_i,'Colors',color_all(i,:),'Widths',0.4,'Symbol','o','OutlierSize',0.5);
    ...
    elseif analysis==2
        ...
        % Algorithm_list = {'Kriging','RBF','NNM-T','Proposed'};
    ...
    elseif analysis==3
        ...
        figure
        for algorithm_idx = 1:1:length(Algorithm_list)
            plot(err_list,frob_algorithms_ratios_all_mean(:,algorithm_idx),'Marker',markers{algorithm_idx},'LineStyle',line_styles{4-mod(algorithm_idx,4)},'Color',color_order(algorithm_idx,:));
            hold on
        end
